  ...
  return (
    <main className="member kort-main">
      ...
      <div className="wrap kort-body">
        <span className="overline">{t("kort.overline")}</span>
        <h1 className="member-title">{t("kort.title")}</h1>
        <p className="kort-intro">{t("kort.intro")}</p>

        <div className="kort-wrap">
          {ready && pinned.length === 0
            ? <div className="kort-empty">{t("kort.empty")}{session ? t("kort.emptyMember") : ""}</div>
            : <MeetMap events={pinned} onSelect={(e) => setOpen(e)} />}
        </div>

        {noPin.length > 0 && (
          <div className="kort-nopin">
            <span className="cp-label">{t("kort.noPinLabel")}</span>
            {noPin.map((e) => (
